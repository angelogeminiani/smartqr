(function(window, SF, $, undefined) {
     ...
     *
     * usage:
     *      var  validator = new SF.forms.Validator();;
     *      // init validator
    ...
    ns.Validator = function(params) {
        ...
        this.data.constraints = new SF.HashMap(); // array of custom functions to run for validation
    ...
    ns.Validator.prototype.validateAll = function(){
        ...
        if(!SF.isEmpty(forms)){
            for(i=0;i<forms.length;i++){
                ...
                result.errors = SF.merge(formResult.errors, result.errors);
            }
        }
        return result;
    };
    ...
    function _initForm(compInstance, form){
        var data = compInstance.data,
            formsel = SF.startsWith(form, "#")?form:"#"+form;

        // add form and fields only if does not exists in internal forms
        if (SF.indexOf(data.forms, form)===-1) {
            // add FORM
    ...
    function _createField(elem, form, required, constraint){
        ...
        elem.min = $(elem).attr('min');
        elem.max = $(elem).attr('max');

        if(!SF.hasText(pattern)){
    ...
    function _getFieldByFormAndName(compInstance, form, name){
        if(SF.hasText(form)&&SF.hasText(name)){
            var fields = compInstance.data.fields,
                i, item;
            for(i=0;i<fields.length;i++){
                item = fields[i];
    ...
    function _getFields(compInstance, formId){
        if(SF.hasText(formId)){
            var fields = compInstance.data.fields,
                result = new Array(),
                fformId = SF.replaceAll(formId, "#", ""),
                i, item;
            for(i=0;i<fields.length;i++){
                item = fields[i];
    ...
    function _validateField(compInstance, field){
        ...
        if(field && field.elem){
            ...
            if(SF.hasText(constraintResponse)){
    ...
    function _validatePattern(pattern, value){
        ...
        // and a )$ at the end."
        var exp = SF.isString(pattern)?new RegExp('^(?:' + pattern + ')$'):pattern;
